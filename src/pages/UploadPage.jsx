...
export default function UploadPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">Upload Event Photos</h1>
      ...
      {uploading && (
        <div className="mt-4">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Uploading & processing faces... {progress}%
          </p>
      ...
      <button
        ...
        className="mt-5 w-full bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700 disabled:opacity-50"
      >
              ...
              <li key={i} className="flex items-center gap-2">
                <span
                  className={`w-2 h-2 rounded-full ${
                    r.status === "success" ? "bg-green-500" : "bg-red-400"
                  }`}
                />
                <span className="text-gray-700">{r.filename}</span>
                {r.status === "success" && (
                  <span className="text-gray-400">
                    — {r.facesDetected} face{r.facesDetected !== 1 ? "s" : ""}
                  </span>
                )}
                {r.status === "error" && (
